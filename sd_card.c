...
//  there is dead code, ugliness and zero error handling... it is very much in a prove it can work
//  state (which it does)
...
static int __time_critical_func(start_single_dma)(uint dma_channel, uint sm, uint32_t* buf,
                                                  uint byte_length, bool bswap, bool sniff) {
    gpio_set_mask(1);
...
static void __time_critical_func(start_chain_dma_read_with_address_size_only)(uint sm,
                                                                              uint32_t* buf,
                                                                              bool bswap,
                                                                              bool sniff) {
    ...
    channel_config_set_write_increment(&c, true);
    channel_config_set_dreq(&c, DREQ_PIO1_RX0 + sm);
    channel_config_set_chain_to(&c,
                                sd_chain_dma_channel); // individual buffers chain back to master
    ...
    dma_channel_configure(sd_chain_dma_channel, &c,
...
static void __time_critical_func(start_chain_dma_read_with_full_cb)(uint sm, uint32_t* buf) {
    ...
    dma_channel_configure(
        sd_chain_dma_channel, &c,
        &dma_channel_hw_addr(sd_data_dma_channel)
             ->read_addr, // ch DMA config (target "ring" buffer size 16) - this is (read_addr,
                          // write_addr, transfer_count, ctrl),                    // dest
        buf,              // src
...
static int __time_critical_func(finish_read)(uint dma_channel, int sm, uint16_t* suffixed_crc,
                                             uint16_t* sniffed_crc) {
    gpio_set_mask(1);
...
int __time_critical_func(sd_response_dma)(uint dma_channel, uint sm, uint32_t* buf,
                                          uint byte_length, bool bswap, uint16_t* suffixed_crc,
                                          uint16_t* sniffed_crc, bool first, bool last,
                                          bool enable) {
    int rc = SD_OK;
    if (first) {
        rc = start_single_dma(dma_channel, sm, buf, byte_length, bswap, sniffed_crc != 0);
        if (!rc)
            rc = start_read(sm, buf, byte_length, enable);
    }
...
int __noinline sd_command(uint64_t packed_command, uint32_t* receive_buf, uint byte_length) {
    ...
    // disable SM so we don't have a race on us filling the FIFO - we must not stall or we will lose
    // sync with clock
    ...
    if (byte_length) {
        rc = sd_response_dma(sd_cmd_dma_channel, SD_CMD_SM, receive_buf, byte_length, false, NULL,
                             NULL, true, true, true);
        if (!rc) {
...
const char* states[] = {"idle", "ready", "ident", "stby", "tran", "data", "rcv", "prg",
                        "dis",  "(9)",   "(a)",   "(b)",  "(c)",  "(d)",  "(e)", "(f)"};
...
static int sd_init(bool _allow_four_data_pins) {
    ...
    uint32_t all_pin_mask =
        (dat_pin_mask << sd_dat_pin_base) | (1u << sd_cmd_pin) | (1u << sd_clk_pin);
    ...
    do {
        ...
        sd_command(sd_make_command(41, 0x40, 0x10, 0, 0), response_buffer,
                   6); // HCS=1, 3.2-3.3V only
        fixup_cmd_response_48(response_buffer);
...
void sd_set_byteswap_on_read(bool swap) { bytes_swap_on_read = swap; }
...
static uint32_t* start_read_to_buf(int sm, uint32_t* buf, uint byte_length, bool first) {
    ...
    if (bus_width == bw_wide && sm != SD_CMD_SM) {
        *buf++ = sd_pio_cmd(sd_cmd_or_dat_offset_state_receive_bits, bit_length / 4 - 1);
        if (first)
            pio_sm_set_wrap(
                sd_pio, sm, sd_cmd_or_dat_offset_wrap_target_for_4bit_receive,
                sd_cmd_or_dat_offset_wrap_for_4bit_receive -
                    1); // note -1since wrap values are on the last instruction before wrap
    } else {
...
int sd_readblocks_scatter_async(uint32_t* control_words, uint32_t block, uint block_count) {
    ...
    dma_channel_configure(sd_pio_dma_channel, &c,
                          &sd_pio->txf[SD_DAT_SM], // dest
                          pio_cmd_buf,             // src
                          buf - pio_cmd_buf, true);
    // todo decide timing of this - as long as dat lines are hi, this is fine. (note this comment
    // now applies to the trigger true in the dma_channel_configure)
    ...
    if (block_count == 1) {
        ...
        if (!rc)
            rc =
                sd_command(sd_make_command(18, block >> 24, block >> 16, block >> 8, block & 0xffu),
                           response_buffer, 6);
    }
    return rc;
}
...
static uint32_t dma_ctrl_for(enum dma_channel_transfer_size size, bool src_incr, bool dst_incr,
    ...
    dma_channel_config c = dma_channel_get_default_config(
        0); // channel doesn't matter as we set chain_to later (it is just use to pre-populate that)
...
int sd_writeblocks_async(const uint32_t* data, uint32_t sector_num, uint sector_count) {
    ...
    for (int i = 0; i < sector_count; i++) {
        // we send an extra word even though the CRC is only 16 bits to make life easy... the
        // receiver doesn't care todo that would need to work anyway for inline CRC (which can't
        // include a pio_cmd)
...
#define build_transfer(src, words, size, flags)                                                    \
    ...
    *p++ = words;                                                                                  \
    *p++ = dma_ctrl_for(size, true, false, DREQ_PIO1_TX0 + SD_DAT_SM, sd_chain_dma_channel, 0, 0,  \
                        true) |                                                                    \
           (flags);
...
//        // fourth cb - transfer sniff
    ...
    *p++ = 0;
    ...
    if (sector_count == 1) {
        rc = sd_command(sd_make_command(24, sector_num >> 24, (sector_num >> 16) & 0xff,
                                        (sector_num >> 8) & 0xff, sector_num & 0xffu),
                        response_buffer, 6);
    } else {
        // todo this is only writing the first sector on SanDisk EDGE 16G right now - probably need
        // a delay between sectors... works fine on a SAMSUNG EVO 32G

        // todo can we expect support for 23?
        rc = sd_command(sd_make_command(23, sector_count >> 24, sector_count >> 16,
                                        sector_count >> 8, sector_count & 0xffu),
                        response_buffer, 6);
        if (!rc)
            rc = sd_command(sd_make_command(25, sector_num >> 24, sector_num >> 16, sector_num >> 8,
                                            sector_num & 0xffu),
                            response_buffer, 6);
    }
    read_status(true);
    if (!rc) {
        ...
        printf("dma chain data (rem %04x @ %08x) data (rem %04x @ %08x) pio data (rem %04x @ %08x) "
               "datsm @ %d\n",
               (uint)dma_hw->ch[sd_chain_dma_channel].transfer_count,
               (uint)dma_hw->ch[sd_chain_dma_channel].read_addr,
               (uint)dma_hw->ch[sd_data_dma_channel].transfer_count,
               (uint)dma_hw->ch[sd_data_dma_channel].read_addr,
               (uint)dma_hw->ch[sd_pio_dma_channel].transfer_count,
               (uint)dma_hw->ch[sd_pio_dma_channel].read_addr, (int)sd_pio->sm[SD_DAT_SM].addr);
    }
    return rc;
}
...
bool sd_write_complete(int* status) {
    printf("dma chain data (rem %04x @ %08x) data (rem %04x @ %08x) datsm @ %d\n",
           ...
           (uint)dma_hw->ch[sd_chain_dma_channel].read_addr,
           (uint)dma_hw->ch[sd_data_dma_channel].transfer_count,
           (uint)dma_hw->ch[sd_data_dma_channel].read_addr, (int)sd_pio->sm[SD_DAT_SM].addr);
    // this is a bit half arsed atm
    bool rc;
    if (dma_channel_is_busy(sd_chain_dma_channel) || dma_channel_is_busy(sd_data_dma_channel))
        rc = false;
...
int sd_read_sectors_1bit_crc_async(uint32_t* sector_buf, uint32_t sector, uint sector_count) {
    ...
    for (int i = 0; i < sector_count; i++) {
        ...
        *p++ = 128;
        *p++ = dma_ctrl_for(DMA_SIZE_32, false, true, DREQ_PIO1_RX0 + SD_DAT_SM,
                            sd_chain_dma_channel, 0, 0, true) |
               DMA_CH0_CTRL_TRIG_SNIFF_EN_BITS | DMA_CH0_CTRL_TRIG_BSWAP_BITS;
        // third crc from stream
        *p++ = (uintptr_t)(&sd_pio->rxf[SD_DAT_SM]);
        *p++ = (uintptr_t)(crcs + i * 2);
        *p++ = 1;
        *p++ = dma_ctrl_for(DMA_SIZE_32, false, false, DREQ_PIO1_RX0 + SD_DAT_SM,
                            sd_chain_dma_channel, 0, 0, true);
    ...
    *p++ = 0;
    ...
    if (sector_count == 1) {
        ...
        if (!rc)
            rc = sd_command(
                sd_make_command(18, sector >> 24, sector >> 16, sector >> 8, sector & 0xffu),
                response_buffer, 6);
    }
    return rc;
}
